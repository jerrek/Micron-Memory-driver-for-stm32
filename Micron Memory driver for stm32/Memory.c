...
void ReinitMemsPinsForOuts (void)//set your pin number
{
...
void ReinitMemsPinsForIns (void)//set your pin number
{
	GPIO_InitTypeDef  GPIO_InitStructure;
...
volatile uint8_t MEMReadData (void)// read 1 byte
{
...
void MEMWriteData (volatile uint8_t byte)// write 1 data byte
{
...
void MEMWriteAdress (volatile uint8_t byte)// write 1 addres byte
{
...
void MEMWriteCommand (volatile uint8_t byte)// write 1 command byte
{
...
uint8_t MEM_Ready_Busy (void)//hardware confirm Ready/Busy
{
	uint8_t status=0;
...
void MEMRead (uint8_t status)// Read mode
{
	if(status==ENABLE)
	{
...
void MEMWrite (uint8_t status)// Write mode
{
	if(status==ENABLE)
	{
...
void MEMAddress (uint8_t status)// Addres Write mode
{
	if(status==ENABLE)
	{
...
void MEMCommand(uint8_t status)// Command Write mode
{
	if(status==ENABLE)
	{
...
void MEMChipSelect (uint8_t status)// Chip Select
{
	if(status==ENABLE)
	{
...
void MEMWriteProtect (uint8_t status)// Protect from write data
{
	if(status==ENABLE)
	{
...
uint8_t ReadStatus(void)// read status byte
{
	uint8_t data=0;
...
void ReadManufacturID( uint8_t *data)// Read ID bytes
{
	uint8_t i=0;
...
void MEMReset(void)// Reset chip
{
	 MEMWriteCommand(0xFF);
	 delay(10000);
}

uint8_t MEMInit(void)// Memory chip init
{
...
void MEMEraseData (uint8_t reset, uint8_t *adr)// Erase 1 block FLASH memory
{
...
void MEMCycleWriteData (volatile uint8_t *byte, uint8_t *adr)// write 1 page FLASH memory
{
...
void MEMCycleReadData (uint8_t reset, uint8_t *adr,  uint8_t *data)// read 1 page FLASH memory
{
	MEMChipSelect(ENABLE);
	MEMWriteProtect(DISABLE);
